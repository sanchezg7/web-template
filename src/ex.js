...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    const changeSettings$ = merge(
        fromEvent(focusTimeInput, "change"),
        fromEvent(focusTimeInput, "keyup"),
        fromEvent(breakTimeInput, "change"),
        fromEvent(breakTimeInput, "keyup")
    );
            ...
            startTimer$.subscribe({
                next: (value) => {
                    timeRemaining.textContent =
                        formatInMinutesAndSeconds(value);
                    setTimer(timer);
                },
                error: (error) => console.log(error),
                complete: () => timer.complete(),
    ...
    function setTimer(timer) {
        ...
        document.title = `${timer.type} time - ${formatInMinutesAndSeconds(
            timer.time
        )}`;
    }
